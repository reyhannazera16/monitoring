import sqlite3
import numpy as np
from datetime import datetime, timedelta
import os

PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
DATABASE_PATH = os.path.join(PROJECT_ROOT, 'backend_laravel', 'database', 'database.sqlite')

def generate_location_data(location, days=365):
    print(f"Generating {days} days of data for {location}...")
    profiles = {
        'Perkotaan': {
            'base_co2': 480, 'trend_co2': 1.5, 'daily_amp_co2': 60, 'noise_co2': 20, 'spike_chance': 0.1,
            'base_co': 2.5, 'trend_co': 0.05
        },
        'Pedesaan': {
            'base_co2': 390, 'trend_co2': 0.1, 'daily_amp_co2': 25, 'noise_co2': 10, 'spike_chance': 0.02,
            'base_co': 1.0, 'trend_co': 0.005
        }
    }
    
    prof = profiles.get(location, profiles['Perkotaan'])
    start_date = datetime.now() - timedelta(days=days)
    hours = days * 24
    timestamps = [start_date + timedelta(hours=i) for i in range(hours)]
    
    data = []
    
    for i, ts in enumerate(timestamps):
        day_number = i / 24
        hour = ts.hour
        weekday = ts.weekday()
        
        trend = day_number * prof['trend_co2']
        daily = prof['daily_amp_co2'] * np.sin((hour - 6) * np.pi / 12) if 6 <= hour <= 18 else -prof['daily_amp_co2']/2
        weekly = 20 if weekday < 5 else -10
        seasonal = 20 * np.sin(day_number * 2 * np.pi / 120)
        noise = np.random.normal(0, prof['noise_co2'])
        spike = np.random.uniform(50, 200) if np.random.random() < prof['spike_chance'] else 0
        
        co2 = prof['base_co2'] + trend + daily + weekly + seasonal + noise + spike
        co2 = max(300, co2)
        
        trend_co = day_number * prof['trend_co']
        daily_co = 2 * np.sin((hour - 6) * np.pi / 12) if 6 <= hour <= 18 else -0.5
        noise_co = np.random.normal(0, 0.4)
        spike_co = np.random.uniform(3, 10) if np.random.random() < (prof['spike_chance'] * 0.8) else 0
        
        co = prof['base_co'] + trend_co + daily_co + noise_co + spike_co
        co = max(0.1, co)
        
        mq7 = 1 if co > 9 else 0
        
        # also add timestamp for created_at, updated_at
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        data.append((
            ts.strftime('%Y-%m-%d %H:%M:%S'),
            location,
            round(co2, 2),
            round(co, 2),
            mq7,
            now,
            now
        ))
    
    return data

def insert_data(data):
    print(f"Inserting {len(data)} records into database...")
    conn = sqlite3.connect(DATABASE_PATH)
    cursor = conn.cursor()
    cursor.executemany('''
        INSERT INTO sensor_readings (timestamp, location, co2_ppm, co_ppm, mq7_detected, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', data)
    conn.commit()
    conn.close()

def main():
    # Clear old data
    conn = sqlite3.connect(DATABASE_PATH)
    conn.cursor().execute('DELETE FROM sensor_readings')
    conn.commit()
    conn.close()
    
    urban_data = generate_location_data('Perkotaan', days=180) # 6 months
    insert_data(urban_data)
    
    rural_data = generate_location_data('Pedesaan', days=180)
    insert_data(rural_data)
    print("Done")

if __name__ == '__main__':
    main()
